#!/usr/bin/env python3
"""
Mol model — ChemBERTa fine-tuned on FAERS SMILES field.

Usage:
    python -m pharmviginet.models.mol --sample 500000   # quick validation run
    python -m pharmviginet.models.mol                   # full training
"""
...
from pharmviginet.config import (
    CHEMBERT, MOL_MAX_LEN, BATCH_SIZE, LR, WEIGHT_DECAY,
...
class MolModel(nn.Module):
    def __init__(self, model_name: str = CHEMBERT, dropout: float = 0.1):
        super().__init__()
        self.bert = AutoModel.from_pretrained(model_name)
        hidden = self.bert.config.hidden_size  # 384 for ChemBERTa
...
def main() -> None:
    ...
    print(f"Loading tokenizer: {CHEMBERT}")
    ...
    train_loader = make_mol_loader(TRAIN_PARQUET, tokenizer,
                                   batch_size=effective_batch,
                                   sample_n=args.sample, shuffle=True)
    val_loader   = make_mol_loader(VAL_PARQUET, tokenizer,
                                   batch_size=effective_batch * 2,
                                   sample_n=sample_val)
    print(f"  train batches={len(train_loader)}, val batches={len(val_loader)}")

    print(f"Loading model: {CHEMBERT}")
    ...
    save_metrics(LOGS / "mol_results.json", results)
